                        ...
                        <div class="card-content">
                            <div class="card-body">
                                ...
                                <div class="card-header">
                                    <h4 class="card-title">Employee Access Rights</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        <form class="form-horizontal" novalidate method="post" action="{{ route('emprights.store') }}">
                                            @csrf
                                            <div class="row">
                                                <div class="col"><strong>Employees</strong></div>
                                            ...
                                            @foreach($Employees as $row)
                                                <div class="row">
                                                    <div class="col">
                                                        {{ $row->name }}
                                                        <input type="hidden" name="employees[]" value="{{ $row->id }}">
                                                    </div>
                                                    @foreach($arrPages as $key=>$val)
                                                        @php
                                                            $strPrevValue = "";
                                                        ...
                                                        <div class="col">
                                                            <fieldset class="form-group">
                                                                @if(array_key_exists($row->id."-".$key, $empRights))
                                                                    @php
                                                                        $arrKey = $row->id."-".$key;
                                                                        $strPrevValue = $empRights["$arrKey"]['rights'];
                                                                    @endphp
                                                                    <input type="hidden" name="{{ $arrKey }}-id" value="@php echo $empRights["$arrKey"]['id'];@endphp">
                                                                @endif
                                                                <select class="form-control" id="{{ $row->id }}-{{$key}}" name="{{ $row->id }}-{{$key}}">
